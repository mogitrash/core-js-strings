...
function isString(value = 0) {
  if (value !== null) return typeof value.valueOf() === 'string';
  return false;
}
...
function isPalindrome(str) {
  ...
  str
    ...
    .split('')
    .forEach((e) => {
      if (e !== ' ' && e !== ',' && e !== '!' && e !== '?') noSpace += e;
    });
  for (let i = noSpace.length - 1; i >= 0; i -= 1) {
    noSpaceReversed += noSpace.at(i);
  }
  return noSpace === noSpaceReversed;
}
...
function findLongestWord(sentence) {
  const lengthArr = sentence.split(' ').map((e) => e.length);
...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    result +=
      str.at(i) === str.at(i).toLowerCase()
        ? str.at(i).toUpperCase()
        : str.at(i).toLocaleLowerCase();
  }
  return result;
}
